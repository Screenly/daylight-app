/**
 * IANA timezone to ISO 3166-1 country, from the tz database's `zone.tab`.
 *
 * The screen's timezone is the only reliable signal of where it is, now that
 * nothing reverse geocodes. The region it yields picks the English conventions
 * for the date: a screen in Berlin writes "Wednesday, 16. September" and one in
 * Chicago writes "Wednesday, September 16", in English either way.
 *
 * Held as one string rather than an object literal to keep 418 entries
 * readable and small. Regenerate from a machine with a current tz database:
 *
 *   grep -v '^#' /usr/share/zoneinfo/zone.tab | awk -F'\t' '{print $3 "=" $1}'
 */
export const ZONE_REGIONS = `
  ...
  Africa/Harare=ZW Africa/Johannesburg=ZA Africa/Juba=SS Africa/Kampala=UG
  Africa/Khartoum=SD Africa/Kigali=RW Africa/Kinshasa=CD Africa/Lagos=NG
  Africa/Libreville=GA Africa/Lome=TG Africa/Luanda=AO Africa/Lubumbashi=CD
  Africa/Lusaka=ZM Africa/Malabo=GQ Africa/Maputo=MZ Africa/Maseru=LS
  ...
  America/Coyhaique=CL America/Creston=CA America/Cuiaba=BR America/Curacao=CW
  ...
  America/Eirunepe=BR America/El_Salvador=SV America/Fort_Nelson=CA
  ...
  America/Nassau=BS America/New_York=US America/Nome=US America/Noronha=BR
  ...
  America/Puerto_Rico=PR America/Punta_Arenas=CL America/Rankin_Inlet=CA
  ...
  America/Thule=GL America/Tijuana=MX America/Toronto=CA America/Tortola=VG
  America/Vancouver=CA America/Whitehorse=CA America/Winnipeg=CA
  America/Yakutat=US Antarctica/Casey=AQ Antarctica/Davis=AQ
  Antarctica/DumontDUrville=AQ Antarctica/Macquarie=AU Antarctica/Mawson=AQ
  Antarctica/McMurdo=AQ Antarctica/Palmer=AQ Antarctica/Rothera=AQ
  Antarctica/Syowa=AQ Antarctica/Troll=AQ Antarctica/Vostok=AQ
  Arctic/Longyearbyen=SJ Asia/Aden=YE Asia/Almaty=KZ Asia/Amman=JO
  Asia/Anadyr=RU Asia/Aqtau=KZ Asia/Aqtobe=KZ Asia/Ashgabat=TM Asia/Atyrau=KZ
  Asia/Baghdad=IQ Asia/Bahrain=BH Asia/Baku=AZ Asia/Bangkok=TH Asia/Barnaul=RU
  Asia/Beirut=LB Asia/Bishkek=KG Asia/Brunei=BN Asia/Chita=RU Asia/Colombo=LK
  ...
  Asia/Macau=MO Asia/Magadan=RU Asia/Makassar=ID Asia/Manila=PH Asia/Muscat=OM
  ...
  Asia/Tbilisi=GE Asia/Tehran=IR Asia/Thimphu=BT Asia/Tokyo=JP Asia/Tomsk=RU
  Asia/Ulaanbaatar=MN Asia/Urumqi=CN Asia/Ust-Nera=RU Asia/Vientiane=LA
  Asia/Vladivostok=RU Asia/Yakutsk=RU Asia/Yangon=MM Asia/Yekaterinburg=RU
  Asia/Yerevan=AM Atlantic/Azores=PT Atlantic/Bermuda=BM Atlantic/Canary=ES
  ...
  Atlantic/Stanley=FK Australia/Adelaide=AU Australia/Brisbane=AU
  Australia/Broken_Hill=AU Australia/Darwin=AU Australia/Eucla=AU
  ...
  Europe/Warsaw=PL Europe/Zagreb=HR Europe/Zurich=CH Indian/Antananarivo=MG
  Indian/Chagos=IO Indian/Christmas=CX Indian/Cocos=CC Indian/Comoro=KM
  Indian/Kerguelen=TF Indian/Mahe=SC Indian/Maldives=MV Indian/Mauritius=MU
  Indian/Mayotte=YT Indian/Reunion=RE Pacific/Apia=WS Pacific/Auckland=NZ
  Pacific/Bougainville=PG Pacific/Chatham=NZ Pacific/Chuuk=FM
  Pacific/Easter=CL Pacific/Efate=VU Pacific/Fakaofo=TK Pacific/Fiji=FJ
  ...
  Pacific/Kiritimati=KI Pacific/Kosrae=FM Pacific/Kwajalein=MH
  Pacific/Majuro=MH Pacific/Marquesas=PF Pacific/Midway=UM Pacific/Nauru=NR
...
/** ISO 3166-1 alpha-2 country for an IANA timezone, or undefined if unknown. */
